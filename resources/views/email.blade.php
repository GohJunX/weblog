   ...
   <ul class="list-group" >
    ...
    <li id="list" class="list-group-item d-flex justify-content-between align-items-center custom-list-item" style="box-shadow: 0px 4px 6px rgba(0, 0, 0, 0.1);margin-bottom:10px;">
        <div>
            ...
            <p class="name"><span class="sender">{{ $email->sender->name }}</span> (Jobseeker) sent you an email for the job application.</p>
        </div>
        <a href="{{ route('notification.show', $email->n_id) }}" id="view-email" class="btn btn-primary view-email-btn">View Email</a>
    </li>
    @endif
    @endforeach
</ul>
